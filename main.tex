...
\maketitle
    \note[itemize]{
        ...
        \begin{enumerate}
            \item SSH direct into kudu in correct directory + test
            \item Correct local directory + test
            \item Teams/camera stuff
        \end{enumerate}
        \item Agenda:
        \begin{enumerate}
...
\begin{frame}{The Rust Language \ i}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item Performant
                \note[item]{No garbarge collection}
                \note[item]{Low-level abstractions\newline}
            \item Productive
                \note[item]{Robust toolchain\newline} % including canonical linters (\texttt{clippy}), auto-formatters (\texttt{rustfmt}), and a centralised package repository (\texttt{crates.io})
            \item Reliable
                \note[item]{Rich type system, pedigree from functional languages}
                \note[item]{Prevents undefined behaviour at compile time\newline}
        \end{itemize}
        \vspace*{0.25cm}
        \item Eliminates ``undefined behaviour'' by compiler rules
            \note[item]{Ownership model, mutable/immutable reference counting}
        \begin{itemize}
...
\begin{frame}{The Rust Language \ ii}
    ...
    \only<3->{
        ...
        \end{figure}
            \note[item]{I pull requested integration tests}
            \note[item]{Proved it is possible to bind functions with array parameters}
            \note[item]{Very common in HPC codebases!}
    }
\end{frame}
...
\begin{frame}{P$^3$HPC - not just performance!}
    ...
    \end{figure}
        \note[item]{Portability and productivity affect calculus\newline}
        \note[item]{C++ very fast}
        \note[item]{Weak portability/productivity with many compilers...\newline}
        \note[item]{Kokkos provides portability/productivity}
        \note[item]{Incurs slight performance overhead\newline}
        \note[item]{Rust similar benefits to Kokkos}
        \note[item]{Slight performance overhead acceptable}
...
pub fn dot_product(
    ...
    lhs.par_iter()
        ...
        .sum()
            ...
            % \only<2> {
            \begin{tikzpicture}[remember picture,overlay]
                \draw[red, very thick] (-0.1,3.1) rectangle (1.9,2.65);
                \draw[red, very thick] (-1.7,3.6) rectangle (0.2,3.15);
    ...
    \begin{itemize}
        ...
        \begin{enumerate}
            \item \alert<2>{End-to-end testing}
                ...
                \note[item]{Can miss edge-cases in complex logic\newline}
            \item Formal methods
                \note[item]{Hoare Logic, doesn't scale well\newline}
            \item LLVM analysis
                \note[item]{Generated LLVM IR may look similar, doesn't scale well\newline}
...
\begin{frame}{Unit Tests with Test-Driven Development}
    ...
    \end{figure}
        \note[item]{TDD writes tests before code to pass them}
        \note[item]{Helpful, but cumbersome in the real world}
\end{frame}
...
\begin{frame}{Agile Workflows}
    ...
    \only<2->{
        \begin{tikzpicture}[remember picture,overlay]
            \node[xshift=-2.6cm,yshift=-1.75cm] at (current page.north east) {\includegraphics[width=0.25\textwidth]{images/github_commit_plot.png}};
        \end{tikzpicture}
        \begin{tikzpicture}[remember picture,overlay]
            \node[xshift=-1cm,yshift=-2.9cm] at (current page.north east) {\includegraphics[width=0.115\textwidth]{images/github_streak.png}};
        \end{tikzpicture}
        \vspace*{-0.75cm}
    }
...
\begin{frame}{Actual Timeline}
    ...
    \begin{figure}[h]
        ...
        \caption{The actual timeline of the project. The progress report was more all-encompassing, parallelisation was easier, and clustering was harder than expected}
            \note[item]{Unforeseen problems in relative difficulty of tasks overcome by flexibility of agile}
