...
% % https://tex.stackexchange.com/a/553261
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item Performant
                ...
                \note[item]{Offers low-level abstraction for machine\newline}
            \item Reliable
...
\begin{frame}{High-Performance Computing}
    The original Mantevo Suite paper \cite{heroux2013mantevo} which pioneered the concept defines mini-apps (also called proxy applications) as:  % Prof Stephen Jarvis was on the development team
    \begin{displayquote}
        \vspace{0.2cm}
...
\begin{frame}[fragile]{Fearless Parallelism in Rust}
    \begin{itemize}
        ...
        \end{itemize}
        \vspace*{0.5cm}
        % TODO: Could compare to PThreads as well?
...
\begin{frame}{Agile Workflows}
    ...
    \begin{itemize}
        \item<1-> Weekly supervisor meetings, which bookend sprints
            \note<1>[item]{Act as both sprint retrospective and planning}
            \note<1>[item]{Agenda written up beforehand, then minutes submitted to Tabula afterwards}
        \item<2-> Agile has empowered the project to be goal orientated and flexible to change \cite{beckManifestoAgileSoftware2001}
            \note<2>[item]{Goal orientation has allowed me to make a number of polished software products}
            \note<2>[item]{Flexibility to change as allowed be to adapt the initial plan based on new information and ongoing supervisor discussion}
        \item<3-> Daily work tracked in \texttt{git}
    \end{itemize}
\end{frame}
